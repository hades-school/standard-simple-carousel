class Carousel {
  constructor(element, options) {
    ...
    this.pauseBtn = this.options.pauseBtn || null;
  ...
  disableActions() {
    this.track.addEventListener("transitionstart", () => {
      ...
      if (this.pauseBtn) this.pauseBtn.style.pointerEvents = "none";
    ...
    this.track.addEventListener("transitionend", () => {
      ...
      if (this.pauseBtn) this.pauseBtn.style.pointerEvents = "auto";
  ...
  setTrackStyle() {
    ...
    this.track.style.gridTemplateColumns = `repeat(${
      this.updatedChildren.length
    }, calc(${100 / this.slidesVisible}% - ${this.minusGap}rem))`;
    this.track.style.transform = `translateX(calc(-${
      this.index * (100 / this.slidesVisible)
    }% - (${this.gap / this.slidesVisible} * ${this.index}rem)))`;
  }

  setDecorate() {
    this.track.childNodes.forEach((image) => {
      if (
        this.slidesVisible !== 1 &&
        image.classList.contains(`slide-${((this.index + 8) % 10) + 1}`)
      ) {
  ...
  swipNext() {
    ...
    this.setDecorate();
    let targetTransform = `translateX(calc(-${
      this.index * (100 / this.slidesVisible)
    }% - (${this.gap / this.slidesVisible} * ${this.index}rem)))`;
    let animate = () => {
  ...
  swipPause() {
    if (this.autoPlayID) this.pause();
    else this.launchAutoPlay();
  }
